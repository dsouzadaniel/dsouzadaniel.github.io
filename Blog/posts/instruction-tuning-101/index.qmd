...
While I try my best to summarize and connect the papers below, this post won’t come close to the joy of reading and
understanding these papers yourself. I highly recommend it! 🙂
...
I will update this appropriately. Thanks and enjoy the journey! 😀
...
As opposed to fine-tuning which requires parameter updates to the model, n-Shot techniques don’t involve gradient updates. From [Brown et al](https://arxiv.org/abs/2005.14165){target="_blank"}, the three broad categories in *decreasing* order of difficulty are :
...
It is crucial to understand what "separation" between the Training and Evaluation data means. In the former case, we’re trying to measure how well the model generalizes to an unseen instance/datapoint, hence the instances i.e datapoints can’t overlap. In the latter setting, since we’re measuring how well the model will generalize to an unseen task, the tasks can’t overlap. So, the training tasks $(T_{seen})$ and evaluation tasks $(T_{unseen})$ are disjoint sets.
...
- <mark>What if we trained on “detailed” instructions ?</mark>
...
<u>Model</u> : They train a **137B**(close in scale to GPT-3, but still smaller) Parameter model that is a LaMDA-PT decoder-style model that was then “instruction-tuned”.
...
💡***Wondering if anyone moved beyond English and built multi-lingual prompt datasets ?*** [xP3](https://huggingface.co/datasets/bigscience/xP3){target="_blank"}(13 tasks in 46 languages) & [xP3x](https://huggingface.co/datasets/Muennighoff/xP3x){target="_blank"}(17 tasks in 277 languages) did that. More recently, there is a massive effort at [C4AI with Project Aya](https://sites.google.com/cohere.com/aya-en/home){target="_blank"} 🌱.

## Natural Instructions

<u>Model</u> : They train a **140M**(tiny compared to GPT-3) Parameter model that is a BART-based encoder-decoder style model. This paper is interesting in that, their findings are more about what “instruction-tuning” could mean and less about how it necessarily *compares* to GPT-3.
...
# Credit
The above post is a humble distillation of the following incredible papers 🙂
